...
利用新的自然语言输入形式，如口语、图表和多媒体输入，为增强LLMs理解和处理多样化用户需求的能力提供了机会。整合口语可以改善开发者和模型之间的交互，实现更自然和丰富的上下文沟通。图表可以促进代码和需求的可视化表示，为代码生成提供补充视角。此外，结合文本、音频和视觉提示的多媒体输入可以提供更全面的理解上下文，从而实现更准确和上下文相关的代码生成。此外，探索基于图的数据集对于解决复杂的代码场景至关重要，因为图可以捕捉代码中的结构关系和依赖，使LLMs更好地理解代码交互和依赖。