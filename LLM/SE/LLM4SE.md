...
### 代码检索
代码检索，是从大型代码库中根据用户的自然语言查询检索源代码的任务。尽管神经网络模型在代码搜索中取得了一定成功，但这些模型通常较浅，无法学习大量数据 [373]。近年来，一些基于 BERT 神经网络架构的双模态预训练模型被提出，以捕捉自然语言和编程语言之间的语义联系 [92, 118, 365, 459]，例如 ​**CodeBERT**​ [92] 和 ​**GraphCodeBERT**​ [118]。这些双模态预训练模型通过设计预训练目标，以无监督的方式从大量数据中学习通用表示。Salza 等人 [373] 探索了 LLMs（如 BERT [373] 和 RoBERTa [40]）在理解自然语言和代码语义以及增强代码搜索和检索方面的有效性。这些研究表明，仅靠预训练任务可能不足以满足代码搜索的需求，强调了需要对数据（包括自然语言和代码）进行多模态理解 [390]。此外，研究表明，使用代码生成模型（如 ​**Codex**​ [219]）可以通过从自然语言文档生成代码片段来增强代码检索，从而提高语义相似性，并在基准数据集上取得了最先进的结果。