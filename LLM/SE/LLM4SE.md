...
提示调优（Prompt Tuning）是一种高效适配预训练模型的技术，通过在输入中添加**可学习的提示标记**​（learnable prompts）引导模型优化任务性能，同时保持模型架构和内部参数固定。其核心原理是设计动态的上下文感知提示，使模型在不调整原始参数的情况下，通过优化少量附加的提示标记来适配特定任务。
