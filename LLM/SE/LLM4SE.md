...
### 错误复现
AdbGPT，利用LLM的自然语言理解和逻辑推理能力从错误报告中提取复现步骤（S2R）实体，并根据当前的图形用户界面（GUI）状态引导错误回放过程。研究人员描述了如何利用提示工程、少量学习和思维链推理来利用LLM的知识进行自动化错误回放。 与传统的利用单个LLM处理 S2R 实体提取的两个阶段以及通过新颖的提示工程进行引导重放的方法相比，这种方法显著轻量。
